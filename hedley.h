...
#if defined(HEDLEY_PREFETCH)
#  undef HEDLEY_PREFETCH
#endif
#if \
  HEDLEY_HAS_BUILTIN(__builtin_prefetch) || \
  HEDLEY_GCC_VERSION_CHECK(3,4,0) || \
  HEDLEY_INTEL_VERSION_CHECK(13,0,0)
#  define HEDLEY_PREFETCH(addr, rw, locality) __builtin_prefetch((addr), (rw), (locality))
#elif defined(__ARM_ACLE)
#  include <arm_acle.h>
#  if (__ARM_ACLE >= 101)
#    define HEDLEY_PREFETCH(addr, rw, locality) __pldx(rw, 0U, HEDLEY_STATIC_CAST(unsigned int, ((locality) < 2)))
#  else
#    define HEDLEY_PREFETCH(addr, rw, locality) __pld((((void) rw), ((void) locality), (addr)))
#  endif
#elif HEDLEY_PGI_VERSION_CHECK(10,0,0)
#  define HEDLEY_PREFETCH(addr, rw, locality) \
    HEDLEY_DIAGNOSTIC_PUSH \
...
#  define HEDLEY_PREFETCH(addr, rw, locality) (__extension__ ({ \
      const void* hedley_addr_ = (addr); \
      if (rw) { \
        if (locality < 2) { \
...
#  define HEDLEY_PREFETCH(addr, rw, locality) ((rw) ? ((locality < 2) ? (sun_prefetch_write_once(addr)) : (sun_prefetch_write_many(addr))) : ((locality < 2) ? (sun_prefetch_read_once(addr)) : (sun_prefetch_read_many(addr))))
#elif HEDLEY_IBM_VERSION_CHECK(11,0,0)
#  define HEDLEY_PREFETCH(addr, rw, locality) __prefetch_by_load((((void) rw), ((void) locality), (addr)))
#elif defined(__SSE__)
#  include <xmmintrin.h>
#  define HEDLEY_PREFETCH(addr, rw, locality) (((void) (rw)), _mm_prefetch(HEDLEY_REINTERPRET_CAST(char const*, (addr)), (locality)))
...
#  define HEDLEY_PREFETCH(addr, rw, locality) (((void) (locality)), ((void) (rw)), __prefetch2((addr), ((locality) > 1)))
#elif defined(_M_ARM)
#  define HEDLEY_PREFETCH(addr, rw, locality) (((void) (locality)), ((void) (rw)), __prefetch((addr)))
#endif
#if !defined(HEDLEY_PREFETCH)
#  define HEDLEY_PREFETCH(addr, rw, locality) (((void) (locality)), ((void) (rw)), ((void) (addr)))
#endif
